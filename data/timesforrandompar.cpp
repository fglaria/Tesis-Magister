...
void getNodeNeighbors(sdsl::wm_int<sdsl::rrr_vector<15>> &x_wm,
    ...
    std::set<uint32_t> &neighs, uint32_t current_node, int32_t nthreads)
{
    const uint32_t howManyX = x_wm.rank(x_wm.size(), current_node);

    omp_set_num_threads(nthreads);
    int tid;
    //std::vector< std::set<uint32_t> > pg;
    std::vector< std::vector<uint32_t> > pg;

    //std::set<uint32_t> gx[nthreads];
    std::vector<uint32_t> gx[nthreads];
    ...
    for (uint32_t xCount = 1; xCount <= howManyX; ++xCount)
    {
        ...
        if(0 == bytesPerNode)
        {
            for (uint64_t xI = partitionIndex; xI < nextPartitionIndex; ++xI)
            {
                if(xIndex != xI)
                {
		    //(pg[tid]).insert(x_wm[xI]);
		    (pg[tid]).emplace_back(x_wm[xI]);
                }
            }
        }
        else
    ...
    for(uint32_t i=0; i<nthreads; i++){
        //std::set<uint32_t> gg = pg[i];
        std::vector<uint32_t> gg = pg[i];
        neighs.insert(gg.begin(), gg.end());
    }
...
int main(int argc, char const *argv[])
{
    if(4 > argc)
    {
        std::cerr << "Modo de uso: " << argv[0] << " RUTA_BASE QueryFile.bin nthreads iterations" << std::endl;
        return -1;
    }
    ...
    for(uint8_t k = 1; k <= iterations; ++k)
    {
        ...
        for(uint32_t i=0;i<queries;i++) {
                ...
                getNodeNeighbors(x_wm, b1_rank, b1_select, b2_wt, y_wm, neighs, qry[i], nthreads);
