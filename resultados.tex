...
\section{Grafos y estructura compacta}
...
En general los grafos contienen una gran mayoría de cliques pequeños y algunos pocos de gran tamaño, a excepción del grafo \texttt{snap-amazon} que no presenta cliques muy grandes. Mención especial requieren \texttt{coPapersDBLP} y \texttt{coPapersCiteseer} los cuales, aún cuando presenta la misma tendencia que los demás, contienen una cantidad considerable de cliques de hasta 100 vértices, a diferencia de los demás grafos donde la mayoría de sus cliques no superan los 25 vértices.
...
Se toman como factores de selección el nivel de compresión en bits y el tiempo de reconstrucción secuencial, usando el Algoritmo~\ref{alg:sequential}, para cada grafo y cada función de ranking con cada una de las estructuras antes planteadas.
...
% En la Tabla~\ref{table:sdslTime} se muestran los tiempos de reconstrucción secuencial del grafo y el espacio en bits para todas las posibles combinaciones. 
...
%En todos los casos, las mejores opciones para las secuencias de símbolos son \textit{wm}, y para la secuencia de bytes es \textit{hutu}. Para la secuencia de bits, usando \textit{rrr} se obtiene menor espacio en bits y con \textit{sdb} se obtiene mejor tiempo de reconstrucción en algunos casos, como marknewman-astro y snap-dblp, pero \textit{rrr} obtiene menor tiempo para dblp-2011 con función $r_{r}(u)$, y para coPapersDBLP con todas las funciones de ranking. Por eso, se elige como estructura de datos para la secuencia de bits \textit{rrr}.

%\input{tables/sdslTime}

%\input{figs/sdslBPEAle}
...
El tiempo de reconstrucción secuencial se midió usando el Algoritmo~\ref{alg:sequential}. En la Tabla~\ref{table:timesSecuencial} se presentan los resultados obtenidos para los algoritmos evaluados. En este caso, si bien el método propuesto es más lento que los demás, para casos como \texttt{marknweman-astro} o \texttt{marknewman-condmat} la diferencia es menor a un segundo, y en otros casos como \texttt{dblp-2010} o \texttt{snap-dblp} es a lo más cercano a dos segundos. El resto de los casos, los grafos tienen una cantidad considerable de arcos, sobre todo \texttt{coPapersDBLP} y \texttt{coPapersCiteseer} que además presentan una relación no proporcional entre cantidad de vértices y cantidad de cliques, como lo muestra la Tabla~\ref{table:gafros3}, lo que afecta bastante a la hora de recuperar el grafo.
